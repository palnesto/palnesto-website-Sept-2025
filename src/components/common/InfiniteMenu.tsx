...
class InfiniteGridMenu {
  ...
  private tex: WebGLTexture | null = null;
  private control!: ArcballControl;
  /** Return the disc (index + screen-space circle) under a canvas pixel, or null. */
  public pickDiscAt(
    px: number,
    py: number
  ): { index: number; cx: number; cy: number; r: number } | null {
    if (!this.gl) return null;

    // We’ll choose the disc whose center is closest to (px, py) AND within its radius.
    // If multiple match, prefer the one most facing camera (largest projected radius).
    let best: { index: number; cx: number; cy: number; r: number } | null =
      null;
    let bestScore = Infinity;

    for (let i = 0; i < this.DISC_INSTANCE_COUNT; i++) {
      const M = this.discInstances.matrices[i];
      if (!M) continue;

      // world = worldMatrix * M
      const W = mat4.create();
      mat4.multiply(W, this.worldMatrix, M as unknown as mat4);

      // Center and a point on +X edge in local space
      const center = vec3.transformMat4(
        vec3.create(),
        vec3.fromValues(0, 0, 0),
        W
      );
      const edge = vec3.transformMat4(
        vec3.create(),
        vec3.fromValues(1, 0, 0),
        W
      );

      const c2 = this.projectWorldToScreen(center);
      const e2 = this.projectWorldToScreen(edge);
      const r = Math.hypot(e2.x - c2.x, e2.y - c2.y);

      // Skip if behind camera (w <= 0) or extremely tiny on screen
      if (c2.w <= 0 || r < 2) continue;

      const dx = px - c2.x;
      const dy = py - c2.y;
      const dist2 = dx * dx + dy * dy;

      // Require pointer to be within radius (with a little tolerance)
      const R = r * 1.2;
      if (dist2 <= R * R) {
        // Score by distance; tie-break by larger radius (more front-facing)
        const score = dist2 - R * 0.01; // small bias
        if (score < bestScore) {
          bestScore = score;
          best = { index: i, cx: c2.x, cy: c2.y, r };
        }
      }
    }
  ...
  /* ---------- projection + hit test helpers ---------- */
  private transformVec4(out: Float32Array, m: mat4, v: Float32Array) {
    out[0] = m[0] * v[0] + m[4] * v[1] + m[8] * v[2] + m[12] * v[3];
    out[1] = m[1] * v[0] + m[5] * v[1] + m[9] * v[2] + m[13] * v[3];
    out[2] = m[2] * v[0] + m[6] * v[1] + m[10] * v[2] + m[14] * v[3];
    out[3] = m[3] * v[0] + m[7] * v[1] + m[11] * v[2] + m[15] * v[3];
  }
  private projectWorldToScreen(p: vec3): { x: number; y: number; w: number } {
    if (!this.gl) return { x: 0, y: 0, w: 1 };
    const c = this.gl.canvas as HTMLCanvasElement;
    ...
    const world = new Float32Array([p[0], p[1], p[2], 1]);
    const clip = new Float32Array(4);
    this.transformVec4(clip, vp, world);
    const ndcX = clip[0] / clip[3];
    const ndcY = clip[1] / clip[3];
    const x = (ndcX * 0.5 + 0.5) * c.clientWidth;
    const y = (1 - (ndcY * 0.5 + 0.5)) * c.clientHeight; // flip Y to CSS pixels
    return { x, y, w: clip[3] };
  }

  /** Returns the active disc’s hit circle in canvas CSS pixels. */
  public getActiveHitCircle(): { cx: number; cy: number; r: number } | null {
    if (!this.gl || this.currentActiveIndex < 0) return null;

    const M = this.discInstances.matrices[this.currentActiveIndex];
    if (!M) return null;

    // world matrix for this instance
    const W = mat4.multiply(
      mat4.create(),
      this.worldMatrix,
    ...
    const edge = vec3.transformMat4(vec3.create(), vec3.fromValues(1, 0, 0), W);

    const c2 = this.projectWorldToScreen(center);
    const e2 = this.projectWorldToScreen(edge);

    const r = Math.hypot(e2.x - c2.x, e2.y - c2.y);
    return { cx: c2.x, cy: c2.y, r };
  }
  ...
  private updateActiveMediaTexture(nowMs: number) {
    ...
    if (item.mediaType === "video") {
      ...
      if (v && v.readyState >= 2) {
        ...
        drawToOffscreenAndUpload(
          v,
          Math.max(v.videoWidth, 1),
          Math.max(v.videoHeight, 1)
        );
      }
    } else if (item.mediaType === "gif") {
      const img = this.images.get(idx);
...
const InfiniteMenu: FC<InfiniteMenuProps> = ({ items = [] }) => {
  ...
  const MOVE_TOL = 10; // px threshold to treat drag (not click)
  const TIME_TOL = 1500; // max ms for "click"
  ...
  useEffect(() => {
    ...
    sketchRef.current = sketch; // 🔥 REQUIRED so events can access instance

    const handleResize = () => sketch.resize();
    window.addEventListener("resize", handleResize);
    handleResize();
    console.log("[init] sketchRef set:", !!sketchRef.current);
    return () => {
      window.removeEventListener("resize", handleResize);
      sketch.dispose();
  ...
  const goToActiveLink = () => {
    const s = sketchRef.current;
    if (!s) return;
    const idx = s.getActiveItemIndex();
    const list = items.length ? items : defaultItems;
    const url = list[idx]?.link?.trim();
  ...
  return (
    <div className="relative w-full text-white">
      <canvas
        ref={canvasRef}
        className={`relative z-[10] w-full h-[80svh] outline-none overflow-hidden ${
          hoverHotspot ? "cursor-pointer" : "cursor-grab active:cursor-grabbing"
        }`}
        onPointerDown={(e) => {
          const rect = (
          ...
          // console.log("[pd] down at", downRef.current);
        }}
        onPointerMove={(e) => {
          const s = sketchRef.current;
          if (!s) return;
          ...
          // 🔎 hover state = over any disc picker hit
          const pick = s.pickDiscAt(px, py);
          setHoverHotspot(!!pick);
        }}
        onPointerLeave={() => {
        ...
        onPointerUp={(e) => {
          ...
          if (performance.now() - start.t > TIME_TOL) return;
          ...
          const pick = s.pickDiscAt(px, py);
          // console.log("[pu] pick", pick);

          if (pick) {
            // Optionally sync the auto-active index to the clicked one:
            // (so your title/cta updates immediately)
            const list = items.length ? items : defaultItems;
            const clicked = list[pick.index % list.length];
            const url = clicked?.link?.trim();
            if (url) openInNewTab(url);
          }
        }}
      />
      ...
      {activeItem && (
        <>
          {/* Non-interactive title so canvas receives clicks */}
          <h2
            className={`pointer-events-none select-none absolute text-3xl lg:text-4xl font-heading-font left-[1.6em] lg:left-1/3 bottom-0 lg:bottom-1/2 transform lg:-translate-x-1/3 translate-y-1/2 transition-all z-10 ${
          ...
          {/* Optional floating button (still works) */}
          <div
            onClick={goToActiveLink}
            className={`absolute left-1/2 z-20 w-[60px] h-[60px] grid place-items-center bg-[#00ffff] border-[5px] border-black rounded-full transition-all ${
              isMoving
                ? "bottom-[-80px] opacity-0 pointer-events-none scale-0 -translate-x-1/2"
                : "bottom-[4.8em] opacity-100 pointer-events-auto cursor-pointer scale-100 -translate-x-1/2"
            }`}
            aria-label="Open project"
            title="Open"
          >
